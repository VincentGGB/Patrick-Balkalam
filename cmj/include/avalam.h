...
octet nbVoisins(octet numCase);      // Retourne le nombre de vosins de la pos envoyée
T_Voisins getVoisins(octet numCase); // Retourne tout les voisins
void listerVoisins(octet numCase);   // Liste tout les voisins
T_Position getPositionInitiale();    // Remet le jeu en pos initiale et retourne la pos de toutes les tours
void afficherPosition(T_Position p); // Affiche la position de chaque tour
void addCoup(T_ListeCoups * pL, octet origine, octet destination); // Permet d'enregistrer un coup (historique)
void afficherListeCoups(T_ListeCoups l); // Afficher l'historique des coups
T_Position jouerCoup(T_Position p, octet origine, octet destination) ; // Fait le déplacement et retourne la nouvelle disposition du jeu
T_ListeCoups getCoupsLegaux(T_Position p); // Retourne tout les coups possibles 
T_Score evaluerScore(T_Position p); // Retourne le score
void afficherScore(T_Score s); // Affiche le score